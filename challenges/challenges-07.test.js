...
const charCode = (arr) => {
  arr.map((val, indx) => {
    arr[indx] = arr[indx].charCodeAt(0);
  })
  return arr;
};
...
const extractAbilities = (arr) => {
  let abilitiesArr = arr.map(val => val.ability.name);
  return abilitiesArr;
};
...
const extractStats = (arr) => {
  return  arr.map(val => val = {name: val.stat.name, total: val.baseStat + val.effort});
};
